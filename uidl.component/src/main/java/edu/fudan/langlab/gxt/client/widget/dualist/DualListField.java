package edu.fudan.langlab.gxt.client.widget.dualist;
...
public class DualListField<M, T> extends AdapterField<List<M>> {
	...
	protected class DualListFieldDefaultMessages implements
			DualListFieldMessages {
	 ...
	 */
	@UiConstructor
	public DualListField(ListStore<M> fromStore, ListStore<M> toStore,
		...
		panel.add(buttonBar);
		panel.add(toView);
		fromView.addDomHandler(new DoubleClickHandler() {

			@Override
			public void onDoubleClick(DoubleClickEvent event) {
				
				if (fromView.getSelectionModel().getSelectedItem() != null)
					onRight();
			}
		}, DoubleClickEvent.getType());
		toView.addDomHandler(new DoubleClickHandler() {

			@Override
			public void onDoubleClick(DoubleClickEvent event) {
				if (toView.getSelectionModel().getSelectedItem() != null)
					onLeft();
			}
		}, DoubleClickEvent.getType());
	...
	/**
	 * Specifies if selections are either inserted or appended when moving
	 * between lists.
	 * 
	 * @param mode
	 *            the mode
