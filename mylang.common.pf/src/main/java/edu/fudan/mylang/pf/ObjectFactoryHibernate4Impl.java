...
public class ObjectFactoryHibernate4Impl implements IObjectFactory, BeanFactoryAware {
	...
	public <T extends IModelObject> T get(Class<T> clazz, Serializable id) {
		IModelObject bean = (IModelObject) getSession().get(getImplClass(clazz), id);
		return (T) bean;
	}

	public IModelObject load(IModelObject e) {
		 getSession().load(e, e.getId());
		return e;
	}
	...
	public <T extends IModelObject> Collection<T> all(Class<T> clazz) {
		...
		return  getSession().createQuery("from " + c.getName() + " where active = 1 ").list();
	}
	...
	public void deleteAll(Class<? extends IModelObject> clazz) {
		for(IModelObject mo :all(clazz)){
			getSession().delete(mo);
		}
	}
	...
	public <S, T extends IQueryCriteria<S>> T createQueryCriteria(Class<T> c) {
		...
		try {
			...
			queryCriteria.setBeanFactory(beanFactory);
			queryCriteria.initialize(beanFactory);
