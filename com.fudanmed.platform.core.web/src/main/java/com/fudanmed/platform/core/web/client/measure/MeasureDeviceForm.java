...
public class MeasureDeviceForm extends GXTFormComponent<UIMeasureDevice> implements IInitializerSupport {
  @Inject
  ...
  protected MeasureDeviceForm doSetValue(final UIMeasureDevice value) {
    this.value = value;
  ...
  public Widget asWidget() {
    final Function1<Void,VerticalLayoutContainer> _function = new Function1<Void,VerticalLayoutContainer>() {
        public VerticalLayoutContainer apply(final Void it) {
          VerticalLayoutContainer _VLayout = MeasureDeviceForm.this.widgets.VLayout();
          final Procedure1<VerticalLayoutContainer> _function = new Procedure1<VerticalLayoutContainer>() {
              public void apply(final VerticalLayoutContainer it) {
                FieldLabel _asWidget = MeasureDeviceForm.this.sid.asWidget();
                WidgetExtensions.<FieldLabel>addFill(it, _asWidget);
                FieldLabel _asWidget_1 = MeasureDeviceForm.this.name.asWidget();
                WidgetExtensions.<FieldLabel>addFill(it, _asWidget_1);
                FieldLabel _asWidget_2 = MeasureDeviceForm.this.location.asWidget();
